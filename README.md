...
DeepSeek Harness（DSH）Web UI 用のクライアント専用プラグイン（DSH `0.1.6-alpha.2` で契約互換性を確認済み、`0.1.7-alpha.2` でプラグイン読み込み・一覧表示・フォント反映を確認済み。後続バージョンは互換性未保証。`@deepseek-ai/dsh-client-store` ベースの Slot store 契約を使用）。
...
DSH `0.1.7-alpha.2` では、プラグイン読み込み・一覧表示・フォント反映を確認済みです。
